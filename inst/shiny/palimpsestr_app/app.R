      ...
      tabItem(tabName = "analysis",
        fluidRow(
          box(title = "Model Parameters", status = "primary", solidHeader = TRUE, width = 4,
              ...
              hr(),
              h5("Feature Space"),
              selectInput("class_model", "Class model",
                          choices = c("Multinomial (recommended)" = "multinomial",
                                      "Gaussian one-hot (legacy)" = "gaussian"),
                          selected = "multinomial"),
              checkboxInput("chrono_precision", "Chronological precision (1/tspan)", FALSE),
              checkboxInput("chrono_uncertainty", "Chronological uncertainty (propagate dating width)", FALSE),
              checkboxInput("strat_dynamic", "Dynamic stratigraphic field (Neighborhood-EM)", FALSE),
              checkboxInput("noise", "Noise component (model-based outlier probability)", FALSE),
              checkboxInput("taf_as_feature", "taf_score as feature", FALSE),
              checkboxInput("residuality", "Residuality (date/context mismatch)", FALSE),
              checkboxInput("class_scale", "One-hot class scaling (Gaussian model only)", FALSE),
    ...
    withProgress(message = "Fitting SEF model...", {
      tryCatch({
        ...
        rv$fit <- fit_sef(rv$data, k = input$k,
                          ...
                          seed = input$seed,
                          class_model = input$class_model,
                          chrono_precision = input$chrono_precision,
                          chrono_uncertainty = input$chrono_uncertainty,
                          strat_dynamic = input$strat_dynamic,
                          noise = input$noise,
