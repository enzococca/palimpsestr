  ...
  for (b in seq_len(n_boot)) {
    ...
    fit_b <- tryCatch(
      suppressWarnings(fit_sef(
        ...
        class_model = null_default(object$class_model, "multinomial"),
        class_smoothing = null_default(object$class_smoothing, 1),
        chrono_uncertainty = null_default(object$chrono_uncertainty, FALSE),
        strat_dynamic = null_default(object$strat_dynamic, FALSE),
        strat_beta = null_default(object$strat_beta, 1),
        noise = null_default(object$noise, FALSE),
        noise_prior = null_default(object$noise_prior, 0.05),
