...
subtitle: "Complete Reference Guide v0.10.0"
author: "Enzo Cocca"
date: "`r Sys.Date()`"
output:
  pdf_document:
...
# From local source
install.packages("palimpsestr_0.10.0.tar.gz", repos = NULL, type = "source")
```
...
| `villa_romana` | 300 | 4 | Realistic | Multi-period Roman villa |
...
The `villa_romana` dataset models a multi-period Roman villa excavation with 300 finds across 4 occupation phases (Republican, Early Imperial, Late Imperial, Late Antique) and 18 stratigraphic units. It includes realistic post-depositional disturbance (bioturbation, construction cuts, residual pottery) and taphonomic scoring.
...
## Validation against known phases

```{r}
adjusted_rand_index(fit_vr, villa_romana$true_phase)
