...
The function \texttt{compare\_k()} was used to evaluate model fit across $K = 2, \dots, 7$, with all v0.12.0 feature-space enhancements activated (chronological precision, residuality, class scaling, taphonomic score as feature). Figure \ref{fig:compare-k} reports the resulting diagnostics: the BIC reaches a clear inflection at $K = 4$, which also coincides with the four macro-events archaeologically distinguishable at the site (Pre-Roman occupation, Late Imperial structural deposits, Late Antique structural collapse, Late Antique cemetery). $K = 4$ was therefore selected as the operational solution. We use the term *macro-event* rather than "phase" deliberately: these four groupings are widely separated in time (collectively spanning $\sim$1100 years) and were chosen to reflect the principal depositional regimes documented on site, not to claim a fine chronological resolution that the data cannot support. Within each macro-event finer subdivisions exist, but their resolution is limited by the precision of the typological dating of the inventoried material.
...
A four-event model with Harris constraint, taphonomic weighting, and the v0.12.0 feature-space enhancements recovered groupings coherent with the site periodisation (Table \ref{tab:vrpg-phases}). Event 1 groups Pre-Roman finds (concentrated in US 210, an early pit beneath the villa foundations) together with residual Late Antique material from levelling deposits; Event 2 captures Late Imperial structural deposits (construction material, amphorae, glass and coins) accumulated during the late phase of habitational use of the villa; Event 3 isolates a small but typologically distinctive group of specialised material (lithics, samples, painted plaster, scraps) related to structural collapse and abandonment; Event 4 (the dominant fraction, 68.8%) corresponds to the Late Antique cemetery, dominated by ceramics and amphorae used in the burial rites.
...
2 & Late Imperial structural deposits & 50 & 8.1 & Construction, amphorae, glass & 0.740 \\
...
- **Type longevity and production-duration estimation**: a derived statistic, computed once the depositional events are estimated, that returns the temporal range during which each cultural class (or sub-class) is associated with non-negligible posterior membership. This would provide a principled, model-based estimate of the **duration of production and use** of pottery types, metal artefact forms, and other typologically defined categories. The same machinery that flags residuality at the find level can be inverted to characterise, at the type level, the chronological window in which the type is most strongly anchored.
