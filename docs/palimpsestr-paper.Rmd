...
- **Direction of intrusions is not yet diagnosed**: the package currently flags finds whose multi-dimensional position is anomalous within their assigned phase, but it does not distinguish between two archaeologically distinct cases. An *older-than-context* find (e.g. a Pre-Roman ceramic in a Late Antique fill) is typically **residual material** redeposited during levelling -- of limited cumulative significance. A *younger-than-context* find (e.g. a Late Antique coin in a Pre-Roman pit fill) is more diagnostically powerful: under the standard archaeological rule that "the most recent find dates the context", such a find may signal an unrecognised intrusion -- a later cut and re-fill missed during excavation, i.e. a **latent feature**. Distinguishing these two regimes is planned for the next release (see *Future development*).
...
Several developments are planned in response to user feedback (notably from M. Cattani, see Acknowledgements) and to the limitations discussed above:

- **Directional intrusion diagnostics** (planned for v0.13.0): a new column in the \texttt{detect\_intrusions()} output classifying flagged finds as *older-than-context* (likely residual material from earlier deposits redeposited during levelling) or *younger-than-context* (likely indicators of unrecognised cuts and refills, i.e. latent features). The distinction is computed by comparing each flagged find's chronological midpoint against the mean midpoint of its stratigraphic unit. This addresses one of the main interpretive needs identified by archaeologists during preliminary use of the package.
- **Validation on dendrochronologically dated sites**: the Poggio Gramignano case study uses periodisation-based dating, with a chronological resolution of $\sim$25--300 years per period. To stress-test the framework on a deposit with annual-resolution chronology, a future case study is planned on the Lago Lucone pile-dwelling site (Gavardo, BS, Italy), where dendrochronological dating provides yearly-resolved temporal anchors for the Bronze Age occupation phases. Such a dataset would allow direct evaluation of how feature-space precision translates into phase-recovery accuracy.
- **Type longevity estimation**: a derived statistic, computed once the latent phases are estimated, that returns the temporal range during which each cultural class (or sub-class) is associated with non-negligible posterior membership. This would provide a principled, model-based estimate of the duration of use of pottery types, metal artefact forms, and other typologically defined categories.
- **Calibrated radiocarbon integration**: support for calibrated $^{14}$C distributions via the \texttt{rcarbon} package (Crema and Bevan 2021), beginning with the cemetery skeletal remains under analysis at Poggio Gramignano.
- **Methodological extensions**: full-covariance and Bayesian variants of the mixture model; spatial-autocorrelation-aware likelihoods; automated detection of sub-typological structure within homogeneous classes; and direct ingestion of OxCal phase models.
...
We thank Professor David Soren (University of Arizona), David Pickel (Stanford University) and the wider Poggio Gramignano excavation team for the long-term archaeological work that produced the dataset analysed here. We are particularly indebted to **Professor Maurizio Cattani** (Università di Bologna) for an in-depth methodological review of an earlier version of this manuscript, whose comments led to the explicit treatment of the horizontal-stratigraphy assumption, the introduction of directional intrusion diagnostics in the development roadmap, and the planned future case study on dendrochronologically dated pile-dwellings. We also thank **Marco Baioni**, director of the Lago Lucone excavation (Gavardo, BS), for opening the possibility of a future collaboration on annually resolved Bronze Age contexts. The development of \texttt{palimpsestr} benefited from the open-source archaeological computing community, particularly the maintainers of \texttt{sf}, \texttt{ggplot2}, \texttt{plotly}, and the PyArchInit project.
